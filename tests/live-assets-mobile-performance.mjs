...
for (const deviceName of devicesToTest) {
  for (const route of routes) {
    ...
    const result = await page.evaluate(async ({ asset }) => {
      const image = [...document.images].find((item) => item.currentSrc.includes(asset) || item.src.includes(asset));
      ...
      await new Promise((resolve) => {
        const tick = (now) => { frameGaps.push(now - previous); previous = now; now < end ? requestAnimationFrame(tick) : resolve(); };
      ...
      return {
        ...
        imageReady,
        imageBytes,
        imageDimensions: image ? { width: image.naturalWidth, height: image.naturalHeight } : null,
        horizontalOverflow: document.documentElement.scrollWidth > innerWidth + 1,
        documentWidth: document.documentElement.scrollWidth,
        approxFps: Number((frameGaps.length / 1.2).toFixed(1)),
        p95FrameGapMs: Number((sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * 0.95))] || 0).toFixed(2)),
        framesOver33ms: frameGaps.filter((gap) => gap > 33.3).length,
        visibleAssetCount: [...document.images].filter((item) => item.currentSrc.includes("/media/radar-assets/") && item.getBoundingClientRect().width > 0).length,
      };
    }, { asset: route.asset });
    results.push({ device: deviceName, route: route.path, expectedAsset: route.asset, loadMs: Date.now() - start, ...result, consoleErrors, pageErrors });
    await context.close();
  }
}
await browser.close();
console.log(JSON.stringify(results, null, 2));
const failed = results.filter((result) => !result.imageReady || result.horizontalOverflow || result.consoleErrors.length || result.pageErrors.length || result.framesOver33ms > 3);
