...
for (const route of routes) {
  ...
  const response = await routePage.goto(`${base}${route}`, {
...
for (const [path, value] of Object.entries(resources))
  if (value.status !== 200) failures.push(`${path}: HTTP ${value.status}`);
for (const route of results) {
  if (route.status !== 200) failures.push(`${route.route}: HTTP ${route.status}`);
  if (!route.title || !route.description)
    failures.push(`${route.route}: missing title or description`);
  if (route.consoleErrors.length || route.pageErrors.length)
    failures.push(`${route.route}: runtime errors`);
}
if (mobile.overflow) failures.push("mobile: horizontal overflow");
if (!results.some((route) => route.ogImage) || !results.some((route) => route.twitterImage))
  failures.push("social previews: og:image and/or twitter:image missing");
if (failures.length) {
  console.error("AUDIT_FINDINGS", JSON.stringify(failures, null, 2));
  process.exitCode = 1;
}
