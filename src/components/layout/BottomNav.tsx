...
export function BottomNav() {
  ...
  return (
    <nav
      className="app-footer-wrapper fixed inset-x-0 bottom-0 z-40 px-3 pb-3 sm:px-6 sm:pb-5 lg:hidden"
      aria-label="Primary"
      style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}
    >
      <div className="app-footer radar-apple-dock mx-auto grid w-fit max-w-full grid-cols-5 gap-1 overflow-hidden rounded-full p-1.5 shadow-[0_25px_50px_-12px_rgb(0_0_0_/_0.7)]">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "footer-item group relative flex min-h-12 min-w-0 items-center justify-center rounded-full px-2 py-2 text-muted-foreground transition-[color,transform] duration-200 sm:min-h-14 sm:px-4",
